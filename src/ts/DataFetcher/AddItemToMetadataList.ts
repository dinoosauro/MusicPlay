 ...
 * @returns the number
 */
export default function GetNewItemInMetadataListPosition({allMetadataLoaded, contentType, isReversed, itemToAdd}: AddItemProps) {
    ...
    mainLoop: while (i < allMetadataLoaded.length) {
        if (allMetadataLoaded[i][1].length !== 0) {
            switch (contentType) {
                case "none":
                    if (allMetadataLoaded[i][1][0].metadata.title.localeCompare(itemToAdd.metadata.title) === (isReversed ? -1 : 1)) break mainLoop;
                    break;
                case "authors":
                    if ((allMetadataLoaded[i][1][0].metadata.artistSort ?? allMetadataLoaded[i][1][0].metadata.artist).localeCompare(itemToAdd.metadata.artistSort ?? itemToAdd.metadata.artist) === (isReversed ? -1 : 1)) break mainLoop;
                    break;
                case "albumauthors":
                    if ((allMetadataLoaded[i][1][0].metadata.albumArtistSort ?? allMetadataLoaded[i][1][0].metadata.albumArtist).localeCompare(itemToAdd.metadata.albumArtistSort ?? itemToAdd.metadata.albumArtist) === (isReversed ? -1 : 1)) break mainLoop;
                    break;
                default:
                    if ((allMetadataLoaded[i][1][0].metadata.albumSort ?? allMetadataLoaded[i][1][0].metadata.album).localeCompare(itemToAdd.metadata.albumSort ?? itemToAdd.metadata.album) === (isReversed ? -1 : 1)) break mainLoop;
                    break;
            }
        }
        i++;
